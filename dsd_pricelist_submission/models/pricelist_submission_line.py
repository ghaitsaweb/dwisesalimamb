...
class PricelistSubmissionLine(models.Model):
    _name = "pricelist.submission.line"
    ...
    pricelist_submission_id = fields.Many2one('pricelist.submission', string="Pricelist Submission", required=True, index=True, ondelete='cascade')
    
    state = fields.Selection(related='pricelist_submission_id.state', store=True)
    ...
    base_pricelist_id = fields.Many2one('product.pricelist', 'Other Pricelist', check_company=True, readonly=True)
